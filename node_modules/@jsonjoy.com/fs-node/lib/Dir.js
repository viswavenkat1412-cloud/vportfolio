"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Dir = void 0;
const fs_node_utils_1 = require("@jsonjoy.com/fs-node-utils");
const argErrors_1 = require("@jsonjoy.com/fs-node-utils/lib/argErrors");
const validators_1 = require("@jsonjoy.com/fs-node-utils/lib/validators");
const Dirent_1 = require("./Dirent");
const util_1 = require("./util");
const DEFAULT_ENCODING = { encoding: 'utf8' };
const dirClosed = () => (0, argErrors_1.withCode)(new Error('Directory handle was closed'), 'ERR_DIR_CLOSED');
const dirBusy = () => (0, argErrors_1.withCode)(new Error('Cannot do synchronous work on directory handle with concurrent asynchronous operations'), 'ERR_DIR_CONCURRENT_OPERATION');
class Dir {
    /**
     * @todo Node builds a Dir on a directory handle with `read(encoding, bufferSize)` and `close()`;
     *     this one walks a `Link` directly, so `new fs.Dir(someHandle, path)` cannot work. Extend
     *     `Superblock` with directory handles.
     *
     * @todo Get rid of `process` (specifically `process.nextTick`) globals in this file. Replace it
     *     by something, or DI inject the "environment" (such as `process`).
     *
     * @param path The path the directory was opened with, kept verbatim as `dir.path`.
     */
    constructor(handle, path, options) {
        this.levels = [];
        this.closed = false;
        this.operationQueue = null;
        if (handle === null || handle === undefined)
            throw (0, argErrors_1.withCode)(new TypeError('The "handle" argument must be specified'), 'ERR_MISSING_ARGS');
        const type = typeof options;
        const given = options === null || options === undefined || type === 'function'
            ? DEFAULT_ENCODING
            : type === 'string'
                ? { encoding: options }
                : options;
        const opts = { bufferSize: 32, ...given };
        (0, validators_1.validateUint32)(opts.bufferSize, 'options.bufferSize', true);
        this.options = opts;
        this._path = path;
        // TODO: Node keeps a Buffer path as a Buffer `parentPath`, but `IDirent['parentPath']` is a string.
        const parentPath = typeof path === 'string' ? path : path.toString();
        this.levels.push({ entries: handle.children[Symbol.iterator](), path, parentPath });
    }
    readBase() {
        const levels = this.levels;
        const options = this.options;
        const encoding = options.encoding;
        const recursive = options.recursive;
        while (levels.length) {
            const level = levels[levels.length - 1];
            const { done, value } = level.entries.next();
            if (done) {
                levels.pop();
                continue;
            }
            const name = value[0];
            if (name === '.' || name === '..')
                continue;
            const link = value[1];
            const dirent = Dirent_1.default.build(link, encoding, level.parentPath);
            if (recursive && dirent.isDirectory())
                this.descend(level.path, dirent.name, link);
            return dirent;
        }
        return null;
    }
    // TODO: Node opens the joined path, failing ENOENT on a `hex` or non-ASCII `latin1` name, we follow the link.
    descend(parent, name, link) {
        const path = (0, fs_node_utils_1.pathJoin)(parent, name);
        if (!link.getNode().canRead())
            throw (0, util_1.createError)("EACCES" /* ERROR_CODE.EACCES */, 'opendir', path);
        this.levels.push({ entries: link.children[Symbol.iterator](), path, parentPath: path });
    }
    readPromise() {
        return new Promise((resolve, reject) => {
            this.readImpl((err, entry) => {
                if (err)
                    reject(err);
                else
                    resolve(entry ?? null);
            });
        });
    }
    readImpl(callback) {
        if (this.closed)
            throw dirClosed();
        if (callback === undefined)
            return this.readPromise();
        const cb = callback;
        (0, util_1.validateCallback)(cb);
        const queued = this.operationQueue;
        if (queued !== null) {
            queued.push(() => {
                try {
                    this.readImpl(cb);
                }
                catch (err) {
                    process.nextTick(cb, err);
                }
            });
            return;
        }
        this.operationQueue = [];
        let entry = null;
        let error = null;
        try {
            entry = this.readBase();
        }
        catch (err) {
            error = err;
        }
        process.nextTick(() => {
            process.nextTick(() => {
                const queue = this.operationQueue;
                this.operationQueue = null;
                if (queue)
                    for (let i = 0; i < queue.length; i++)
                        queue[i]();
            });
            if (error)
                cb(error);
            else
                cb(null, entry);
        });
    }
    // --------------------------------------------------------------------- IDir
    get path() {
        // TODO: brand-check like Node's `#path in this`: `instanceof` lets `Object.create(Dir.prototype)` through.
        if (!(this instanceof Dir))
            throw (0, argErrors_1.invalidThis)('Dir');
        return this._path;
    }
    close(callback) {
        if (callback === undefined) {
            if (this.closed)
                return Promise.reject(dirClosed());
            return new Promise((resolve, reject) => {
                this.close((err) => {
                    if (err)
                        reject(err);
                    else
                        resolve();
                });
            });
        }
        const cb = callback;
        (0, util_1.validateCallback)(cb);
        if (this.closed) {
            process.nextTick(cb, dirClosed());
            return;
        }
        const queued = this.operationQueue;
        if (queued !== null) {
            queued.push(() => {
                this.close(cb);
            });
            return;
        }
        this.closed = true;
        process.nextTick(cb, null);
    }
    closeSync() {
        if (this.closed)
            throw dirClosed();
        if (this.operationQueue !== null)
            throw dirBusy();
        this.closed = true;
    }
    read(callback) {
        if (arguments.length === 0)
            return this.readPromise();
        return this.readImpl(callback);
    }
    readSync() {
        if (this.closed)
            throw dirClosed();
        if (this.operationQueue !== null)
            throw dirBusy();
        return this.readBase();
    }
    async *entries() {
        try {
            while (true) {
                const entry = await this.read();
                if (entry === null)
                    break;
                yield entry;
            }
        }
        finally {
            await this.close();
        }
    }
    async [Symbol.asyncDispose]() {
        if (this.closed)
            return;
        await this.close();
    }
    [Symbol.dispose]() {
        if (this.closed)
            return;
        this.closeSync();
    }
}
exports.Dir = Dir;
Object.defineProperty(Dir.prototype, Symbol.asyncIterator, {
    enumerable: false,
    writable: true,
    configurable: true,
    value: Dir.prototype.entries,
});
//# sourceMappingURL=Dir.js.map